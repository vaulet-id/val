...
///
/// The fixed operations are the same whatever is being run — a pure function on
/// its own, or an action with a host behind it — so they are defined once and
/// the two runners differ only in what they add.
pub(crate) trait HasValues {
    fn values(&self) -> Shared;
}

impl HasValues for Shared {
    fn values(&self) -> Shared {
        self.clone()
    }
}

/// Define every fixed `val` operation on this linker.
pub(crate) fn define_ops<T: HasValues + 'static>(
    store: &mut Store<T>,
    linker: &mut Linker<T>,
    konsts: Vec<Value>,
) -> Result<(), String> {
    macro_rules! binop {
        ($name:literal, $f:expr) => {{
            let ks = ();
            let _ = ks;
            let func = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32, b: i32| -> Result<i32, wasmi::Error> {
                let vals = caller.data().values();
                let (x, y) = {
                    let v = vals.borrow();
                    (v.get(a), v.get(b))
                };
    ...
    let ks = konsts.clone();
    let konst_fn = Func::wrap(&mut *store, move |caller: Caller<'_, T>, i: i32| -> i32 {
        let v = ks.get(i as usize).cloned().unwrap_or(Value::Null);
        caller.data().values().borrow_mut().put(v)
    });
    ...
    let len = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> i32 {
        match caller.data().values().borrow().get(a) {
    ...
    let at = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32, i: i32| -> i32 {
        let vals = caller.data().values();
    ...
    let count = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> i32 {
        let vals = caller.data().values();
    ...
    let first = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> i32 {
        let vals = caller.data().values();
        let item = match vals.borrow().get(a) {
    ...
    let exists = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> i32 {
        let vals = caller.data().values();
    ...
    let set = Func::wrap(
        &mut *store,
        move |caller: Caller<'_, T>, a: i32, b: i32, c: i32| -> i32 {
            let vals = caller.data().values();
    ...
    let not = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> i32 {
        let vals = caller.data().values();
    ...
    let neg = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> Result<i32, wasmi::Error> {
        let vals = caller.data().values();
        let v = vals.borrow().get(a);
        match v {
    ...
    let truthy = Func::wrap(&mut *store, move |caller: Caller<'_, T>, a: i32| -> i32 {
        i32::from(caller.data().values().borrow().get(a).truthy())
    });
...
pub fn run_with_fuel(module: &Module, name: &str, args: &[Value], fuel: Option<u64>) -> Result<Value, String> {
    ...
    define_ops(&mut store, &mut linker, konsts)?;
