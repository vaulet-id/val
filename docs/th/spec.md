...
`storage.write`, `message.send` และ `network.request` เคยอยู่ในทะเบียนของ host และใน
รายการนี้ โดยไม่มี back end ไหน emit ให้เลย และสองในสามยังเถียงกับสิ่งที่ภาษานี้ตัดสินไป
แล้ว — `state` คือสิ่งที่ถูกแฮชและ replay ที่เก็บที่สองนอกมันจึงเป็น state ที่ record ไม่
ครอบ และ `api.query` มีอยู่ก็เพื่อไม่ให้แอปเอื้อมถึงเครือข่ายเอง ทั้งสามถูกลบออก ไม่ใช่ถูก
ทำให้ใช้ได้ ส่วน `message.send` จะกลับมาในวันที่มีอะไรส่งข้อความจริง พร้อมทั้ง lowering
และ host ที่ทำมันได้ในการเปลี่ยนแปลงเดียวกัน

การประกาศ capability ที่ host นี้ไม่มี ตอนนี้เป็น error **บนบรรทัดที่เขียน** แทนที่จะเป็น
build ที่ล้มโดยไม่มีบรรทัด
