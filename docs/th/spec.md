...
**URL เป็นของผู้ออกเอง ภาษานี้ไม่มีทะเบียน** ตรวจแค่ว่าเป็น absolute และ `https`
เท่านั้น ผู้ออกที่รันบนโดเมนตัวเองก็เขียนโดเมนตัวเอง ซึ่งเป็นกรณีปกติ ไม่ใช่ข้อยกเว้น
ที่ต้องหาทางเลี่ยง
...
แอปส่วนใหญ่เปิดให้ทุกคนที่ถือเครื่องอยู่ บางแอปไม่ใช่:
...
ถ้าไม่มี credential ที่ผ่าน `EmployedByAcme` แอปนี้จะไม่วาดหน้าจอแรกและไม่รัน
action ผู้ใช้เห็น `notStaff` ซึ่งเป็น key ใน text bundle คำพูดจึงเป็นของผู้เผยแพร่
ผ่านการทบทวนและแปลเหมือนทุกประโยคในแพ็กเกจ

**host เป็นคนตอบที่ประตู โปรแกรมไม่เคยถาม** ในภาษานี้เขียนว่า "คนนี้ถืออันนี้อยู่
ไหม" ไม่ได้ และการที่เขียนไม่ได้คือตัวการออกแบบ: โปรแกรมที่ถามได้คือโปรแกรมที่ต้อง
ถือ credential ไว้เพื่อจะรู้ว่ามันไม่มี ซึ่งคือการอ่านที่ประตูนี้มีไว้แทน สิ่งที่แอป
ที่ผ่านประตูรู้คือมันเปิดแล้ว

ประตูจึงเป็น `credential.check` ไม่ใช่ `credential.read` — ความต่างที่ sheet
แปลงเป็นสองประโยคคนละแบบเกี่ยวกับแอปเดียวกัน ประกาศว่า:

```val
capabilities { credential.check(EmployeeBadge) }
```

ทั้งสองครึ่งของบรรทัดเป็นสิ่งบังคับ ไม่มี policy ประตูก็รับอะไรก็ได้ที่รูปร่าง
เหมือนบัตร รวมถึงบัตรที่ใครทำขึ้นเอง ไม่มีประโยคประตูก็ปิดเงียบๆ แล้วผู้ใช้เหลือแค่
รายงานความผิดพลาดแทนคำแนะนำ ทุกบรรทัดต้องผ่าน — สองประตูคือสองเงื่อนไข ไม่ใช่
ตัวเลือก เพราะประตูที่เปิดให้ credential อย่างใดอย่างหนึ่งคือ policy เดียวเหนือ
credential เดียว เขียนไว้ในที่ที่คนตรวจเห็นว่ามันเป็นอย่างนั้น

ประตูเป็นสิ่งเดียวบน sheet ที่ host **ประเมิน** แทนที่จะ **วัด** สิ่งที่ทำให้มัน
ซื่อสัตย์จึงเป็นการตรวจตอน install: credential ที่มันระบุต้องเป็นตัวที่ import
section ของ module บอกว่าแอปนี้มองอยู่ ประตูบน sheet ที่ไม่มีในโค้ดถูกปฏิเสธในฐานะ
sheet ที่ผิด
...
`limit` จำเป็นสำหรับ list ที่คุณคำนวณต่อ — `map` `filter` `fold` `any` `all`
ส่วน `count` กับ `first` อ่านโดยไม่เดินทั้งรายการ มันจำกัดปริมาณงาน ซึ่งเป็นสิ่งที่ทำให้
ผลรวมของ list คอมไพล์ลงไปเป็นวงจรได้ และทำให้การพิสูจน์จ่ายตามขอบเขต ไม่ใช่ตามข้อมูล

**การกดระบุชื่อ action** handler ทุกตัวเป็นแบบนั้น — `onTap` และ `onRemove` ของ list —
ทุกอย่างที่หน้าจอเริ่มได้จึงผ่านหกเฟสด้วยความยินยอมชุดเดียวกันและ record ชุดเดียวกัน
ส่วนว่า prop ไหนเป็น handler เป็นคำตอบของ registry ของ host ไม่ใช่ชื่อที่เขียนไว้ในคอมไพเลอร์
...
`default` ของฟิลด์ใน state เป็นค่าที่เขียนออกมาตรงนั้น มันถูกอ่านก่อนที่อะไรจะรัน:
ไม่มี scope ให้อ่านชื่อ และไม่มีเวอร์ชันก่อนหน้าให้ถาม
...
**ไม่มี source เดินทางไปใน package** เครื่องของผู้ใช้ไม่มีคอมไพเลอร์ — การคอมไพล์
บนมือถือไม่ใช่สิ่งที่ใครเขา ship กัน — สิ่งที่มันได้รับคือสิ่งที่รัน และทุกอย่างที่มันตรวจคือ
การตรวจไบต์ชุดนั้น แต่มันก็ไม่ได้เชื่ออะไรฟรีๆ: สิ่งที่แอปทำได้คือ **import section**
ของ module ซึ่งอ่านออกจากไบต์ได้ตรงๆ และ module เอื้อมถึงได้เฉพาะสิ่งที่มัน import

ส่วนคำถามว่า module นี้มาจาก source ที่ประกาศไว้จริงไหม ตอบด้วย **reproducible
build** ไม่ใช่ด้วยการส่ง source ไปทุกเครื่อง: source เดิมกับคอมไพเลอร์เดิมให้ไบต์ชุดเดิม
ใครอยากตรวจก็ build แล้วเทียบ ครั้งเดียว
...
1. module แฮชออกมาตรงกับที่ integrity บอก
2. ลายเซ็นครอบไบต์ชุดนี้ โดยกุญแจที่ manifest ระบุชื่อ
3. มัน import เฉพาะสิ่งที่ host ตัวนั้นมีให้ — ชื่อที่ host ไม่รู้จักคือการปฏิเสธ ไม่ใช่การ
   link เพราะไม่อย่างนั้นรายการที่บอกว่ามันทำอะไรได้ก็เลิกเป็นทั้งหมดของสิ่งที่มันทำได้
4. รายงานที่มันส่งมาคือรายงานที่ module ของมันผลิต
5. ทุกประตูที่มันประกาศเป็นประตูที่ module ของมันมี `check` import รองรับ และประโยคที่
   ประตูที่ปิดแสดงอยู่ใน bundle
6. ทุกภาษาที่ manifest สัญญาไว้มีครบทุก key
...
.val sources                                      │   .vapp  (module + manifest
     ...
     ├─ การวิเคราะห์ capability และความเชื่อถือ    │        ├─ ผู้เผยแพร่เซ็น bytes ชุดนี้
     ├─ determinism และ totality                  │        ├─ import section คือ report
     │                                            │        └─ รัน
...
iOS ห้าม JIT runtime จึงเป็นแบบตีความ — และนั่นคือเหตุผลเดียวกันที่ไม่มีอะไร
บนเครื่องผู้ใช้คอมไพล์อะไรทั้งนั้น

module หอบสิ่งที่ execution record ต้องใช้ไปด้วยข้างๆ โค้ด: แอปไหน เวอร์ชันอะไร
capability ที่ประกาศ policy ที่มี credential ที่แต่ละ action ขอ และค่าเริ่มต้นของ
state — เครื่องเติม record ได้จาก bytes อย่างเดียว
